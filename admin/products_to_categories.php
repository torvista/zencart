...
if (isset($_POST['target_category_id'])) {
    ...
    if (isset($_GET['target_category_id'])) {
...
{
    global $db, $categories_info, $debug_p2c;//steve debug
    ...
    while (!$categories_result->EOF) {
        ...
        $category_name = (strlen($category_path_string) > 0 ? $category_path_string . ' > ' : '') .
            $categories_result->fields['categories_name'];
...
function ceonGetTargetCategoryList($parent_id = '0', $spacing = '', $category_tree_array = '')
{
    ...
    while (!$categories->EOF) {
      ...
      FROM
        " . TABLE_CATEGORIES . " c
      WHERE
        c.parent_id = '" . (int)$categories->fields['categories_id'] . "';";
        ...
        if (!$sub_categories_check_result->EOF) {
            ...
            $category_tree_array = ceonGetTargetCategoryList($categories->fields['categories_id'],
...
function ceonGetTargetCategoryProductList($parent_id = '0', $spacing = '', $category_product_tree_array = '')
{
    ...
    while (!$categories->EOF) {
      ...
      WHERE
        ...
        if (!$sub_categories_result->EOF) {
            $category_product_tree_array = ceonGetTargetCategoryProductList(
                $categories->fields['categories_id'], $spacing . $categories->fields['categories_name'] .
      ...
      WHERE
        p.master_categories_id = " . $categories->fields['categories_id'] . "
      AND
      ...
      ORDER BY
        ...
        while (!$products_result->EOF) {
            if ($products_result->fields['products_id'] != $products_filter) {
                $category_product_tree_array[] = array(
                    'id' => $products_result->fields['products_id'],
                    'text' => $spacing . $categories->fields['categories_name'] . ' : ' .
                        $products_result->fields['products_name'] . ' #' . $products_result->fields['products_id']
...
//          $cnt_added++;
        ...
        //remove products from TO category that are linked from FROM category
...
// END CEON MODIFICATIONS 1.2.0 8 of 28
            break;
// BEGIN CEON MODIFICATIONS 1.2.0 9 of 28
        case 'set_target_category':
            $target_category_id = $_POST['target_category_id'];

            zen_redirect(zen_href_link(FILENAME_PRODUCTS_TO_CATEGORIES, 'products_filter=' .
        ...
        case 'copy_linked_categories_to_another_product':
            ...
            if ($target_product_id == '') {
                $messageStack->add(WARNING_COPY_LINKED_CATEGORIES, 'error');
            } else {
                // Get the master category for the source product
                $source_product_master_category_sql = "
            SELECT
            ...
            WHERE
              products_id = '" . (int)$products_filter . "';";
                ...
                $target_product_master_category_sql = "
            SELECT
              master_categories_id
            FROM
              " . TABLE_PRODUCTS . "
            WHERE
              products_id = '" . (int)$target_product_id . "';";
                ...
                if (!$target_product_master_category_result->EOF &&
                    !$source_product_master_category_result->EOF) {
                    // Remove all of the target product's current category links, except for its master
                    // category
                    $target_product_master_categories_id =
                        $target_product_master_category_result->fields['master_categories_id'];

                    $source_product_master_categories_id =
                        $source_product_master_category_result->fields['master_categories_id'];

                    $clear_target_product_categories = $db->Execute(
                        "DELETE FROM
              ...
              WHERE
                products_id = '" . (int)$products_filter . "';");
                    ...
                    while (!$product_categories_result->EOF) {
                        ...
                        // Don't duplicate the target product's master category linking and don't link the
                        // target product into the source product's master category
                        if ($category_id != $target_product_master_categories_id &&
                            $category_id != $source_product_master_categories_id) {
                            $db->Execute("
                  INSERT INTO
                    " . TABLE_PRODUCTS_TO_CATEGORIES . "
                    (
                  ...
                  VALUES
                    ...
                    '" . $category_id . "'
                        ...
                        $product_categories_result->MoveNext();
                    }

                    $target_product_name_sql = "
              SELECT
                products_name
              FROM
              ...
              AND
                language_id = '" . (int)$_SESSION['languages_id'] . "';";
                    ...
                    $messageStack->add(sprintf(SUCCESS_LINKED_CATEGORIES_COPIED_TO_TARGET_PRODUCT, $target_product_name_result->fields['products_name']), 'success');
                }
            }
...
<head>
    ...
    <style>
        select#target_product_id { /* steve: hack to limit the width of the "Copy Linked Categories to Another Product" drop-down surrounding container or enormous option values push page layout to the right */
            width: 600px;
...
//      $contents[] = array('align' => 'center', 'text' => '<br />' . zen_image_submit('button_update.gif', IMAGE_UPDATE) . '&nbsp;<a href="' . zen_href_link(FILENAME_PRODUCTS_TO_CATEGORIES, 'products_filter=' . $products_filter . '&current_category_id=' . $current_category_id) . '">' . zen_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>' . '</form>');
        ...
        <!-- RIGHT column block (prev/next, product select, master category) -->
    </div>
    <!-- product-selection-info block eof -->
...
{
  var select_all_or_none_el = document.getElementById('select_all_or_none');
...
JS_BLOCK;
                    ...
                    while (!$products_list->EOF) {
                        $selected_categories[] = $products_list->fields['categories_id'];
    ...
    <div class="col-lg-12">
        ...
        <div class="row dataTableHeadingRow">
            <?php echo zen_draw_form('linked_copy', FILENAME_PRODUCTS_TO_CATEGORIES,
                'action=copy_categories_products_to_another_category_linked' . '&products_filter=' . $products_filter . '&current_category_id=' . $current_category_id, 'post',
        ...
        <div class="row dataTableHeadingRow">
            <?php echo zen_draw_form('linked_remove', FILENAME_PRODUCTS_TO_CATEGORIES,
                'action=remove_categories_products_to_another_category_linked' . '&products_filter=' . $products_filter . '&current_category_id=' . $current_category_id, 'post',
                'class="form-horizontal"'); ?>
            <h3><?php echo TEXT_HEADING_REMOVE_ALL_PRODUCTS_FROM_CATEGORY_LINKED; ?></h3>
            <div class="form-group-row">
                <?php echo TEXT_INFO_REMOVE_ALL_PRODUCTS_TO_CATEGORY_LINKED; ?>
            </div>
            <div class="form-group-row">
                <div class="col-lg-4">
        ...
        <div class="row dataTableHeadingRow">
            ...
            <div class="form-group-row">
                ...
                <div class="col-lg-4">
                    <button type="submit" class="btn btn-primary"><?php echo BUTTON_COPY_LINKED_CATEGORIES; ?></button>
                </div>
            </div>
            <?php echo '</form>'; ?>
        </div>
